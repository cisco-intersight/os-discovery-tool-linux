...
cmd="${ipmitoolcmd} raw 0x36 0x77 0x03 0x68 0x6f 0x73 0x74 0x2d 0x69 0x6e 0x76 0x2e 0x79 0x61 0x6d 0x6c"
$cmd

echo "[localhost]: Getting File Descriptor For host-inv.yaml inventory file from IMC"
filedescriptor=$(${ipmitoolcmd} raw 0x36 0x77 0x00 0x68 0x6f 0x73 0x74 0x2d 0x69 0x6e 0x76 0x2e 0x79 0x61 0x6d 0x6c)
...
for (( i=0; i<${#filebytearray[@]}; i++ )); do
    ...
    if [ $counter -le 39 ]; then
        ...
        cmd="${ipmitoolcmd} raw 0x36 0x77 0x02 "$filedescriptor" "$payloadlength" "$filepointer" 0x00 0x00 "$payload
...
filepointer=$(printf "0x%04X" $filelocationpointer)
filepointer=$(echo $filepointer | sed -E 's/0x(..)(..)/0x\2 0x\1/')
cmd="${ipmitoolcmd} raw 0x36 0x77 0x02 "$filedescriptor" "$(printf '0x%x\n' $counter)" "$filepointer" 0x00 0x00 "$payload
$cmd

echo "[localhost]: Closing IMC file handle"
cmd="${ipmitoolcmd} raw 0x36 0x77 0x01 "$filedescriptor
$cmd
